...
bool CInputDialogImport::push_browse_file(void)
{
    QString sFile;
    QStringList ls;
    QFileDialog dlg(this,QString("Datei öffnen"));
    //-
    ...
    if(file.exists())
    {
        if(file.open(QIODevice::ReadOnly|QIODevice::Text))
        {
            ...
            while (!file.atEnd())
            {
                ...
                if(s.length()>0)
                {
                    if(s[s.length()-1]=='\n')//only rows - last char in row = '\n'
                    {
                        s.remove(s.length()-1,1);//remove \n
...
bool CInputDialogImport::set_error_msg_step1(bool bError)
{
    if(bError)
    {
        ...
        ui->label_error_1->setText(QString(""));
    }
    else
    {
        ui->label_error_1->setVisible(true);
        ui->label_error_1->setText(QString("Es wurde keine Datei ausgewählt."));
    }
    return true;
}
...
bool CInputDialogImport::update_view(void)
{
    ...
    for(j=0;j<ui->tableWidgetFileView->rowCount();j++)
    {
        for(i=0;i<ui->tableWidgetFileView->columnCount();i++)
        {
            ...
            {
                if(ui->tableWidgetFileView->item(j,i)->text()!=QString(""))
                {
                    b=true;
                    break;
                }
...
bool CInputDialogImport::tranfer_fileview_data(void)
{
    ...
    else
    {
        for(rs=0,r=0;rs<ui->tableWidgetFileView->rowCount();r++,rs++)
        {
            ...
            s=QString("");
            for(c=0;c<ui->tableWidgetFileView->columnCount() && s==QString("");c++)
            {
...
bool CInputDialogImport::table_fileview2_item_changed(QTableWidgetItem* pItem)
{
    ...
    if(pItem!=NULL && m_sMarkValueUserEditFileview2.length()>0)//edit by user?
    {
        ...
        if(sInput.length()>0)
        {
            if(ui->comboBoxTyp->currentIndex()==0)//type article? (has only number types)
            {
                ...
                if(y<ui->tableWidgetAllocation->rowCount())
                {
                    ...
                    if(!b)
                    {
                        s=QString("Ihre Eingabe muss eine %1 sein! Siehe Spaltenzuordnung und Typ.").arg(s);
                        msg.setText(s);
                        msg.exec();
        ...
        m_sMarkValueUserEditFileview2=QString("");//clear mark
    }
    return true;
}
    ...
    if(ls.count()>2)
    {
        if(ls[0]!=NULL && ls[1]!=NULL && ls[2]!=NULL)
        {
            ...
            if(ls[2]->text()!=QString(""))
            {
                ...
                if(!b)
                {
                    ls[2]->setText(QString(""));
                    iValue=0;
                }
            }

            sDataName=QString("Spaltenzuordnung bearbeiten:   Datenfeld : '%1'   Typ: %2").arg(ls[0]->text(),ls[1]->text());
            c=QInputDialog::getInt(this,QString("Spaltenzuordnung bearbeiten"),sDataName,iValue,0,iMaxValue,1,&b);
            if(b)//press ok?
            {
                if(c==0)//disable?
                {
                    if(ui->tableWidgetAllocation->item(ls[0]->row(),2)!=NULL)
                        ui->tableWidgetAllocation->item(ls[0]->row(),2)->setText(QString(""));//clear item
                    else{}
                }
                else
                {//ok
                    ...
                    if(i<ui->tableWidgetAllocation->rowCount())
                        msg.setText(QString("Diese Spaltennummer ist bereits von einem anderen Datenfeld gesetzt!"));//set error
                    else
                    {
                        //check type
                        ...
                        {//error
                            s=QString("Die Daten in der Spalte '%1' sind keine ").arg(c);
                            if(ls[0]->row()>=0 && ls[0]->row()<14)//in array range?
                            {
                                if(iaArticleTyp[ls[0]->row()]==1)//integer type?
                                    s+=QString("Ganzzahlen!");
                                else//double type?
                                    s+=QString("Kommazahlen!");
                            }
                            msg.setText(s);
                        }
    ...
    if(ui->comboBoxTyp->currentIndex()==0)//actile? (has only number type's)
    {
        if(ui->tableWidgetFileView_2->columnCount()>=0 && iSourceColumn<ui->tableWidgetFileView_2->columnCount())//column in range?
        {
            if(iMemberRow>=0 && iMemberRow<14)//member row in range? (array)
            {
                ...
                {
                    for(i=0;i<ui->tableWidgetFileView_2->rowCount() && b==true;i++)
                    {
...
bool CInputDialogImport::check_allocation_settings(void)
{
    ...
    {
        if(ui->tableWidgetAllocation->item(0,2)->text()!=QString(""))//name datafield column set?
            b=true;
    }
...
bool CInputDialogImport::update_allocation_table(void)
{
    ...
    {
        case 0://article
            lsMember.push_back("Bezeichnung");
            lsMember.push_back("Bestand");
            lsMember.push_back("max.Lagerkapazität");
...
bool CInputDialogImport::set_error_msg_step2(bool bError)
{
    if(bError)
    {
        ...
        ui->label_error_2->setText(QString(""));
    }
    else
    {
        ui->label_error_2->setVisible(true);
        ui->label_error_2->setText(QString("Das Datenfeld Bezeichnung bzw. Name muss einer Spalte zugeordnet sein."));
    }
    return true;
}
...
bool CInputDialogImport::update_record_table(void)
{
    ...
    for(y=0;y<ui->tableWidgetFileView_2->rowCount();y++)
    {
...
bool CInputDialogImport::check_record_table_data(void)
{
    ...
    if(ui->tableWidgetRecordData->rowCount()<=0)//rows?
       sError=QString("Es gibt keine Daten in der Tabelle 'Vorschau importierender Datensätze'!");

    //Error? -> set text
    if(sError.length()>0)
    {
        s=QString("<font color='darkRed'>%1</font>").arg(sError);
    ...
    {
        ui->label_error_3->setText(QString(""));
        ui->label_error_3->setVisible(false);
        bEnable=true;
    }
...
bool CInputDialogImport::table_record_item_changed(QTableWidgetItem* pItem)
{
    ...
    if(m_bCheckItemDataRecordTable==true)//check enable
    {
        ...
        {
            sError=QString("Kein Datenfeld in der Spalte Name bzw. Bezeichnung darf leer sein!");
            b=false;
        }
        ...
        if(b==true && pItem!=NULL)
        {
            ...
            if(ui->comboBoxTyp->currentIndex()==0)//index 0 = type article -> has only number types
            {
                ...
                if(c>=0 && c<14)
                {
                    ...
                    //input error
                    if(!b)
                        sError=QString("Ihre Eingabe muss eine %1 sein! Siehe Spaltenzuordnung und Typ.").arg(s);
                }
            }
            ...
            if(ui->comboBoxTyp->currentIndex()!=3)//check by names (index 3 = customer -> name indifferent)
            {
                ...
                if(pItem->column()==0 && b==true)//column name?
                {
                    ...
                    {
                        case 0://article
                            sCondition=QString("name = '%1' AND delete_status=0").arg(sInput);//search not delete article
                    ...
                    if(c>0)//found in db
                    {
                        sError=QString("Einen Datensatz vom Typ '%1' mit dem Namen bzw. Bezeichnung ").arg(sDataTypeName);
                        sError+=QString("'%1' gibt es schon in der Datenbank.").arg(sInput);
                        b=false;
                    }
                    ...
                    if(b)
                    {
                        for(c=0;c<ui->tableWidgetRecordData->rowCount();c++)
                        {
                            if(ui->tableWidgetRecordData->item(c,0)!=NULL && c!=pItem->row())
                            {
                                ...
                                {
                                    sError=QString("Einen Datensatz mit dem Namen bzw. Bezeichnung '%1' ").arg(sInput);
                                    sError+=QString("gibt es schon in der Tabelle in Zeile %1.").arg(c+1);
                                    b=false;
                                    break;
                                }
                            }
        ...
        //-
        m_sMarkValueUserEditRecordTable=QString("");//clear mark
    }
    return b;
}
    ...
    if(b==true && iDataType!=3)//type 3==customer - name indifferent
    {
                    ...
                    if(lsDoubleInTable[r]->count()>0 && lsDoubleInTable[r]->first()!=-1)
                    {
                        if(ui->tableWidgetRecordData->item(r,0)!=NULL)
                        {
                            ...
                            for(k=0,i=0;k<lsDoubleInTable[r]->count();k++)
                            {
                                ...
                                {
                                    if(i>0)
                                        sName_new=sName+QString("_%1").arg(i);
                                    //-
                                    switch(iDataType)
                                    {
                                        case 0://article
                                        sCondition+=QString("name = '%1' AND delete_status=0").arg(sName_new);//search not delete article
                                        c=m_pThread->m_pDbInterface->article_get_count(sCondition);
                                        break;
                                        case 1://dealer
        ...
        for(r=0;r<ui->tableWidgetRecordData->rowCount();r++)
        {
            if(ui->tableWidgetRecordData->item(r,0)!=NULL)
            {
                ...
                for(c=1,i=0,sCondition=QString("");c>0;i++)
                {
                    if(i>0)
                        sName_new=sName+QString("_%1").arg(i);
                    //-
        ...
        if(bTableUpdate)
        {
            ui->label_error_3->setText(QString("<font color='black'>Einige Namen bzw. Bezeichnungen wurden angepasst! (Grund: Mehrfachvergabe)</font>"));
            ui->label_error_3->setVisible(true);
        }
        else
        {
        ...
        if(m_pThread->m_pDbInterface!=NULL)
        {
            if(iType==0)//only type article
            {
                ...
                if(lsMark.count()>0)
                {
                    if(lsMark.count()==1)
                    {
                        s=QString("Der Hersteller '%1' existiert nicht in der Datenbank!").arg(lsMark[0]);
                        msg.setText(s);
                        msg.setInformativeText("Soll dieser Hersteller angelegt werden?");
                    }
                    else
                    {
                        s=QString("Es gibt %1 Hersteller die in der Datenbank nicht existieren!").arg(lsMark.count());
                        msg.setText(s);
                ...
                if(j>0)
                {
                    s=QString("Es gibt %1 Warengruppen die in der Datenbank nicht existieren!").arg(j);
                    msg.setText(s);
