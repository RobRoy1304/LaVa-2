...
bool CDbConnection::write_logbook_count(int iNewCount)
{
    ...
    {
        s=QString("UPDATE info SET logbook_count = %1").arg(iNewCount);
        b=query.exec(s);
    }
    return b;
}
...
bool CDbConnection::check_and_update_db_version(void)
{
    ...
    else
    {
        ...
        {
            if(sDbVersion==QString("1,01"))//update to 1,02
            {
            ...
            if(sDbVersion==QString("1,02"))//update to 1,03
            {
                //new column in info
                s=QString("ALTER TABLE info ADD COLUMN last_change TEXT");
                bReturn=exec_sql(s);

                if(bReturn)
                {
                    s=QString("ALTER TABLE info ADD COLUMN logbook_count INTEGER DEFAULT (100)");
                    bReturn=exec_sql(s);

                    if(bReturn)
                    {
                        //update db_version text
                        s=QString("UPDATE info SET version = '1,03'");
                        bReturn=exec_sql(s);
                    }
                }
            }
...
bool CDbConnection::write_last_change(QString sLastChange)
{
    ...
    if(m_bConnect)
    {
        s=QString("UPDATE info SET last_change='%1'").arg(sLastChange);
        b=query.exec(s);
    }
    return b;
}
    ...
    if(m_bConnect==true && sWares.length()>0)
    {
        ...
        for(i=0;i<lsWares.count();i++)
        {
            ...
            if(id>0)//cast ok?
            {
                ...
                sCondition=QString("id = %1").arg(id);
                c=article_get_count(sCondition,bWithDeleteArticle);
                //-
                if(c>0)
                {//article found
                    sWaresFound+=QString("%1x").arg(count);
                    sWaresFound+=QString("%1|").arg(id);
                }
            }
        }
        //-
