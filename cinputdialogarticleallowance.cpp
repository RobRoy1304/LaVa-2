...
/*
void CInputDialogArticleAllowance::timerEvent(QTimerEvent *event)
{
    QString s;
    int id,type;
    CArticle ar;
    ...
    if(event!=NULL && m_pThread!=NULL)
    {
        ...
        {//db change from another client?
            ...
            if(m_LastDbChange.get_data_from_last_change_string(type,id,s))//split string
            {
                ...
                {//update row (by id)
                    if(id<=0)
                        type=LC_ACTION_UPDATE_ALL;
                    else
                    {
                        if(m_pThread->m_pDbInterface->article_get(id,ar))//get article
                        {
                            m_pThread->m_pWidgets->article_update_row(ui->tableWidgetArticle,ar,FORMAT_TWO,false,false);
                        }
                    }
                }
                //-
                if(type==LC_ACTION_UPDATE_ALL || type==LC_ACTION_UPDATE_INV_ORD_TRADE_ARTICLE_MK_WG)
                {//update all rows
                    update_table();
                }
            }
...
bool CInputDialogArticleAllowance::set(CWorkThread * pThread, QTableWidget * pParentTable, int iColumnCount, int iColumnId, int iType, int iArticleId, QDate dtDate)
{
    ...
    {//edit-mode
        ...
        ui->pushButtonCancel->setText(QString("abbrechen"));
...
void CInputDialogArticleAllowance::press_ok(void)
{
    ...
    {//value ok?
        ...
        else
        {
            ...
            ui->textBrowser->setText(QString("Artikel zur Warenliste hinzugefügt."));
            ui->tableWidgetArticle->clearSelection();
            //-
            ui->spinBox->setFocus();
            ui->spinBox->findChild<QLineEdit*>()->setSelection(0,1);//select text value
        }
    }
}
