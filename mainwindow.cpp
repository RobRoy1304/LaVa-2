    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(ui->menuDbImportFromLavaOne,SIGNAL(triggered()),this,SLOT(menu_db_import_from_lava1()));
...
void MainWindow::set_user(QString s)
{
    ...
    if(s.length()>0)
    {
        title=QString("lava2 - user: %1").arg(s);
...
void MainWindow::timerEvent(QTimerEvent *event)
{
    ...
    static QString mark_error_string=QString("");

    bool bQuit=false;
    QMessageBox msg(QMessageBox::Critical,"","");
    QMessageBox msg2(QMessageBox::Information,"","");
    msg2.setWindowTitle(QString("Achtung!"));
    msg2.setText(QString("Eine andere Programminstanz ist dabei eine Sicherung zu laden, ein Neustart des Programmes zur Wiederherstellung der Datenbankverbindung ist nötig!"));
    QString s,s2,title;
    QDate dt=QDate().currentDate();
    int type,id;
    ...
    //title back
    title=windowTitle();
    if(title.indexOf(QString("user"))<0)
    {
        title=QString("lava2 - user: %1").arg(m_sUser);
        setWindowTitle(title);
    }
    ...
    {
        //info in main title
         setWindowTitle(QString("lava2 - Datenbank wurde von einer anderer Programminstanz verändert..."));
...
bool MainWindow::article_copy(void)
{
    ...
    msg.setText(QString("Der Artikel der als Quelle dienen sollte wurde zwischenzeitlich von einem anderen client gelöscht."));
...
bool MainWindow::logbook_insert(CLogbook & lg)
{
    ...
    {
        s=QString("user: %1 - %2").arg(m_sUser,lg.get_text());
        lg.set_text(s);
    }
...
bool MainWindow::menu_db_backup_create(void)
{
    ...
    if(sBackupPath.length()>0)//backup path load?
        ...
        sWriteFile=sDbPathFolder+QString("/%1.sqlite").arg(dtTi.toString(QString("hh-mm-ss_dd-MM-yyyy")));
...
bool MainWindow::menu_db_backup_load(void)
{
    ...
    msgQuest.setText(QString("Achtung, beim Laden einer Sicherung gehen alle aktuellen Daten unwiderruflich verloren."
                             " Außerdem werden alle anderen Programminstanzen die auf diese Datenbank zugreifen aufgeforderten, sich zu beenden.\nMöchten Sie wirklich eine Sicherung laden?"));
    ...
    for(int i=0;i<ls.count()-1;i++)
        sDbPathFolder+=ls[i]+QString("/");
    ls.clear();
    sWriteTempFile=sDbPathFolder+QString("lava_temp.sqlite");//set temp file path
    ...
    if(!file_info.permission(QFile::WriteUser))
        sError=QString("Sie haben keine Schreibrechte für den  Ordner \"%1\"!").arg(sDbPathFolder);//error
    else
    {
        ...
        if(msgQuest.clickedButton()==yesButton)
        {
            ...
            {//load button pressed?
                ...
                if(sReadFile==m_sDbPath)
                    sError=QString("Sie benutzen zur Zeit die Datenbankdatei \"%1\"!").arg(m_sDbPath);//error
                else
                {
...
{
    QList<QString> ls;
    ls.push_back(QString("Bst./max.Kap."));
    ls.push_back(QString("Artikelbez."));
    ls.push_back(QString("1/2.Artikelnr."));
    ls.push_back(QString("Hersteller"));
    ls.push_back(QString("Warengruppe"));
    ls.push_back(QString("Standort/Kommentar"));

    CDlgUniList dlg;
    dlg.setWindowTitle(QString("Inventurliste"));
    dlg.set_thread(&m_thread);
...
bool MainWindow::menu_import_from_csv(void)
{
    ...
    if(dlg.exec())
    {
        ...
        lg.set_type(LOGBOOK_TYPE_OTHER);
        lg.set_date_time(QDateTime::currentDateTime());
        s=QString("Daten aus der Datei \"%1\" in die Datenbank importiert").arg(dlg.get_source_file());
        lg.set_text(s);
        logbook_insert(lg);
...
bool MainWindow::print_table(QString sPageTitle, QString sError, QTableWidget * pTable, bool * pAllColumns)
{
    ...
    bool ok;
    QString s=QInputDialog::getText(this,QString("Überschrift ändern"),QString("Überschrift:\t\t"),QLineEdit::Normal,sPageTitle,&ok);
    if(ok)
        sPageTitle=s;
