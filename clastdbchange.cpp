...
QString CLastDbChange::create_last_change_string(int iType, int iId)
{
    ...
    if(iType>=0)
    {
        sReturn=QString::fromUtf8("%1|").arg(dt.toString("hh:mm:ss-dd.MM.yyyy"));//date&time
        sReturn+=QString::fromUtf8("%1|").arg(iType);//type
        sReturn+=QString::fromUtf8("%1").arg(iId);//id
    }
    //-
    return sReturn;
}
